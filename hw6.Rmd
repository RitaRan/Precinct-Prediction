...
x_span = seq(-73.981900, -73.949300, 0.00001)
y_span = seq(40.764400,40.800600,0.00001)
x = sample(x_span, 10000,replace = TRUE)
y = sample(y_span, 10000,replace = TRUE)
...
central = cbind(precinct = as.integer(precinct),x,y) %>% as_data_frame %>% .[.$precinct==22,]
...
m = xgboost(data=x, label=y, nthead=4, nround=20, objective="multi:softmax", num_class=length(precincts))
